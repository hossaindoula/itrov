(function($){ 
	$(document).ready(function(){
		...
		$('#theme_switcher').change(function(){
			if($(this).val() == 1)
				var style = $('<link rel="stylesheet" href="http://hogash-demo.com/kallyas_wp/wp-content/themes/kallyas/css/dark-theme.css" type="text/css" id="theme_style" />').appendTo('head');
			else if($(this).val() == 0)
				$('#theme_style').remove();
		});
		...
		function changeColor(hex) {
			style.html(
				...
				'#action_box:before,header#header.style2 #logo a {border-top-color:' + hex + ';}'+
				...
				'.iosSlider .item .caption.style2.fromright .title_big, .iosSlider .item .caption.style2.fromright .title_small , .iosSlider .item .caption.style2.fromright .title_big, .iosSlider .item .caption.style2.fromright .title_small {border-right: 5px solid ' + hex + '; }');
		}
